    ...
    class Program
    {
        ...
        static MyResultSet getFirst(int numOfNN)
        {
            ...
            for (int i = 0; i < numOfNN; i++)
            {
                ...
                //resultSet = (MyResultSet)Program1.ResultSet.GetRange(0, numOfNN);
            resultIndex = numOfNN;
        ...
        static MyResultSet getNext(int numberOfNode)
        {
            ...
            if ((resultIndex + numberOfNode) > Program1.ResultSet.Count)
            {
                while ((Program1.ResultSet.Count < (resultIndex + numberOfNode)) && (Program1.prunedNodeQueue.Count != 0))
                {
                    ...
                    {
                        MyResultEntry resultEntry = new MyResultEntry { filename = Program1.prunedNodeQueue[0].fileID };
                        if (!Program1.ResultSet.Contains(resultEntry))
                        {
                            Program1.ResultSet.AddTo(Program1.prunedNodeQueue[0].fileID, Program1.prunedNodeQueue[0].pruneDistance);
                        }
                    ...
                    { 
                        ...
                        {
                            Program1.findRecursivelyInPruned(Program1.QueryPoints[i], Program1.prunedNodeQueue[0]);
                        }
                    }
                }
            }
